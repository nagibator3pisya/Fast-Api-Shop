...
from app.deps.dependes_admin import get_current_admin_user
...
async def update_categories(id_category: int, categoryes: CategoryUpdate,
...
async def update_products(id_products: int, product_schemas: ProductUpdate,
                          session: AsyncSession = Depends(get_db),
                          current_user=Depends(get_current_admin_user)):
...
@admin_router.post('/add_adm/{user_id}')
async def add_admins(user_id: int, current_user=Depends(get_current_admin_user),
                     session: AsyncSession = Depends(get_db)):
    result = await service_admins.add_admins(user_id=user_id, session=session)
    return result
